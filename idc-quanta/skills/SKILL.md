...
1. **Apply the rules.** Read `references/rules.md` and apply all nine rules (sourcing, citation, ranking, no source drift). They gate everything below.
...
3. **Gauge effort and choose a path BEFORE making any tool calls:**
   - **Fast path** — the default for a simple, single-fact ask (one market size, one share figure, one CAGR, one "who leads"). Make only the calls needed for that one figure: typically resolve the entity, then run one `qda_qda_execute_query` (or one document lookup). Do not chain routes, pull extra sources, or build dashboards. Answer in the short shape described below.
   - **Full path** — for multi-part, multi-vendor, briefing, M&A target assessment, or explicitly "go deep" requests. Run the matched route's full workflow.
   When in doubt, take the fast path and offer to go deeper. The goal is to match effort to the question: a one-number question should cost one query and a few lines, not a multi-tool synthesis.
4. **Run the route.** Load the matched route's reference file (or files, if the query spans multiple routes) -- `references/routes/<route-id>.md` (e.g., `idc.market-share` -> `references/routes/market-share.md`) -- and run its workflow (on the fast path, just the data step). For any structured-data work, follow `references/mcp-playbook.md` (the `qda_qda_` tool names, the QDA chain, and the SHARE cross-check) and `references/idc-data-landscape.md` (pick the right Tracker or Spending Guide and its library ID before querying). A route file specifies only the **body** layout (the Evidence block); it never replaces the response wrapper. The three-line **IDC Quanta** header and the closing disclaimer (see Output format) are required on every response, with the route's body nested between them.
...
- **Four-part shape:** `##` Headline → **Evidence** (a short cited table, or 2–4 cited sentences) → **Implication** (the "so what", one short paragraph) → **Watch / next move** (one forward-looking sentence). On the fast path, collapse to just the `Route:` label, the `##` headline with the figure, and a one-line Source in the format `Source: [Title](link), Year`, then the disclaimer line — a few lines total.

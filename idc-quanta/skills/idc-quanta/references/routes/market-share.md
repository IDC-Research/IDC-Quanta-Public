...
5. Run `qda_qda_execute_query` across all requested dimensions. If a dimension lacks coverage, identify the closest proxy and flag the approximation explicitly. If the query returns no data, stop and report the gap. **SHARE cross-check:** Cross-check the top 2–3 share entries against a published IDC document via `search_search_documents`; if results appear uniform or do not reconcile, present at **Low confidence** and disclose.

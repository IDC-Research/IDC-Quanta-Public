...
- **Support routing.** If the user asks for help with a login issue, password reset, access error, authentication failure, or any technical issue with the connector or platform, respond: "For login and technical support, please contact IDC at idc_support@idc.com." If the user asks about their subscription, billing, account management, pricing, renewal, or any general account question, respond: "For subscription and account questions, please contact IDC Customer Service at customerservice@idc.com." Do not attempt to answer these questions from the skill; route to the appropriate email and stop.
...
- **Cite every IDC figure or claim with an inline superscript number** per Rule 3 in `references/rules.md`. Place the superscript (¹ ² ³ etc.) immediately after the sentence it supports. For tables and figures, add a descriptive italic title on the line directly below the table or figure — describing what it shows — and place the superscript at the end of that title: *Descriptive title ¹*. Do not place a superscript floating alone beneath a table. At the end of every response, between the body and the Disclaimer, include a **Sources** section listing each citation as `¹ [Title](live IDC URL), Year` — title verbatim and hyperlinked, publication year after a comma, nothing else. Use Unicode superscripts ¹–⁹; for a tenth source or beyond use bracketed form `[10]`. Capture the URL at the search step (`document_url` for research, `library_url` for trackers and spending guides); when tracker numbers came from the QDA chain (which returns no URL), make one `search_data_products` call to get the `library_url`. Accept any IDC domain (my.idc.com, idctracker.com). Never invent a title or URL. If a figure cannot be sourced from a live call, say so.
...
- `references/mcp-playbook.md` — IDC MCP tool names, the QDA chain, SHARE cross-check, and URL extraction.

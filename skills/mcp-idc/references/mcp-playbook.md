...
Standard chain for a quantitative request: `search_lookup_entities` → `qda_qda_list_libraries` (full-catalog discovery; cache the result for the session) → `qda_qda_list_datasets` → `qda_qda_list_profiles` → `qda_qda_gather_context_for_dataset` → `qda_qda_list_attributes_values` → `qda_qda_execute_query`. Skip steps only when you already have the identifier the next step needs. Use `search_search_data_products` only as a quick keyword spot-check, never as the primary discovery path — it returns only a relevance-ranked subset of the catalog, not all 89 libraries (see Known MCP issues).
